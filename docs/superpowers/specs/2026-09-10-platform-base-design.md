...
- `packages/storage`: `ObjectStore` interface with GCS and in-memory implementations, shared by API and worker.
...
- Deployment to Cloud Run, Cloud SQL, GCS and Cloud Tasks via `infra/bootstrap.sh` and `infra/deploy.sh` (gcloud); GitHub Actions CI/CD.
...
| Package manager / orchestration | pnpm 11 workspace, Turborepo |
...
**Creation (API).** Inside one DB transaction: insert the job row with `state = queued` and an idempotency key `{type}:{subject_id}:{pipeline_version}`. If the key already exists, return the existing job (idempotent). After commit, enqueue a Cloud Tasks task whose `name` is `{queue}/tasks/{job.id}-{attempt}` so a duplicate enqueue returns `ALREADY_EXISTS` and is ignored, while a later retry gets a fresh name. Task body: `{ jobId }`. Target: `POST {WORKER_URL}/tasks/{type}` with an OIDC token for the worker's service account audience.
...
`infra/bootstrap.sh` creates these idempotently with gcloud; `infra/deploy.sh` builds and pushes images, runs the migrate job, and deploys both services with `gcloud run deploy`.
...
**CI (`.github/workflows/ts.yml`).** On push/PR: `pnpm install --frozen-lockfile`, `turbo lint typecheck test build` with a Postgres service container. On push to `main`: `infra/deploy.sh` builds and pushes images, runs the `maester-migrate` job, then deploys worker and api. Auth via Workload Identity Federation. The existing Python `check.yml` is unchanged.
